...
// Получаем URI из query string (для shared хостинга)
$request_uri = isset($_GET['_route']) ? '/' . $_GET['_route'] : $_SERVER['REQUEST_URI'];
...
if ($request_uri === '/' || $request_uri === '/index.html' || $request_uri === '') {
    $file_path = $base_dir . '/static/index.html';
} else {
    $file_path = $base_dir . '/static' . $request_uri;
}
